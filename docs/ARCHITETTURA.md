...
- `/api/condizioni`: `/elenchi` (articoli, letture, arcani, Persona, abilità, Ladri, attività, negozi, eventi, contatori — gli elenchi chiusi dell'editor); `/partite/:id/progressi` e i `PUT .../eventi/:chiave`, `.../attivita/:chiave`, `.../punti-negozio/:chiave` (scheda **Partita → Progressi**, `ProgressiPartita.tsx`). Frontend: `guida/CondizioniEditor.tsx` (righe `[NON] [Stato ▾] [operatore] [valori]`, blocchi TUTTE/ALMENO UNA, numeri a passi) e `condizioni/SelettoreRicerca.tsx` (elenco chiuso con ricerca scrivendo, riusabile per ogni selettore lungo).
...
- FE: `src/services/api/impostazioni.ts` (nome del file dal `Content-Disposition`) e `src/components/impostazioni/BackupIstanza.tsx` in fondo a Impostazioni: stato dell'istanza, «Scarica l'istanza completa», «Scarica solo il database», «Ripristina da file» con conferma; dopo il ripristino ricarica le partite nello store.
