...
const [dbArg,planArg,atlasArg,reportArg,backupOption]=process.argv.slice(2);
if(backupOption && backupOption!=='--no-backup')throw new Error('Opzione non valida.');
...
const backup=backupOption==='--no-backup'?null:dbPath+'.before-organization-'+crypto.randomUUID()+'.sqlite';
if(backup)await db.backup(backup);
let committed=false;
try {
 db.transaction(()=>{
