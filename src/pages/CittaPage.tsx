    ...
    <PageState isLoading={dati.caricamento && !q} error={dati.errore} onRetry={() => void dati.ricarica()}>
      {q && (
        <div className="flex flex-col gap-4">
          ...
          <MappaIncorporata onNaviga={(k,arrivo)=>{if(arrivo){navigate(urlMappa(k,arrivo));return;}const quartiere=q.find(v=>v.mappaChiave===k);navigate(quartiere?`/guida/mondo/quartiere/${encodeURIComponent(quartiere.chiave)}`:`/guida/mappe/${encodeURIComponent(k)}`);}} chiave="tokyo" altezza="max(520px, calc(100vh - 260px))" />
