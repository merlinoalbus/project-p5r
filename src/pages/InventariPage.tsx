...
export function InventariPage() {
  ...
  const carte = useMemo(() => (articoli.dati?.articoli ?? []).filter((a) => `${a.nome} ${a.nomeIt ?? ''} ${a.effetto ?? ''}`.toLocaleLowerCase('it').includes('cart')), [articoli.dati]);
  ...
  else if (categoria === 'carte') contenuto = <>{carte.length ? <ArticoliTabella articoli={carte} partitaId={partitaId} mostraNegozio onCambiato={() => void articoli.ricarica()} /> : <section className="card"><p>Nessuna carta abilità nel catalogo locale; la fonte ufficiale resta consultabile.</p><a href="https://www.allgamestaff.it/persona-5-royal/carte-abilita/" target="_blank" rel="noreferrer" className="credito">Fonte carte abilità</a></section>}</>;
