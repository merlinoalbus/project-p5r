...
export function LibriPage() {
  ...
  return <PageState isLoading={dati.caricamento && !d} error={dati.errore} onRetry={() => void dati.ricarica()}>
    {d && <div className="flex flex-col gap-4">
      ...
      <p className="m-0 text-sm text-text-secondary">{visibili.length} libri mostrati</p>
      <ul className="m-0 grid list-none gap-3 p-0 md:grid-cols-2 xl:grid-cols-3" aria-label="Libri">
        {visibili.map((libro) => {
          ...
          return <li key={libro.chiave} className={`card relative flex min-w-0 flex-col gap-3 overflow-hidden ${libro.fatto ? 'border-success/50' : ''}`}>
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1"><h2 className="m-0 text-lg leading-tight">{libro.nomeIt ?? libro.nome}</h2>{libro.nomeIt && libro.nomeIt !== libro.nome && <p className="m-0 text-xs text-text-muted">{libro.nome}</p>}</div>
              <span className={`chip ${libro.fatto ? 'chip--attivo' : ''}`}>{libro.fatto ? 'Completato' : progresso ? 'In corso' : 'Da leggere'}</span>
            </div>
            <div><div className="mb-1 flex justify-between text-xs text-text-secondary"><span>{progresso} di {libro.totaleSessioni} sessioni</span><span>{percentuale}%</span></div><div className="visore-mappa__progresso" role="progressbar" aria-label={`Progresso ${libro.nomeIt ?? libro.nome}`} aria-valuemin={0} aria-valuemax={libro.totaleSessioni} aria-valuenow={progresso}><span className="visore-mappa__progresso-barra" style={{ width: `${percentuale}%` }} /></div></div>
            {partitaId && <div className="grid grid-cols-[44px_1fr_44px] gap-2" aria-label={`Avanzamento ${libro.nomeIt ?? libro.nome}`}>
              <button type="button" className="btn btn-ghost touch" disabled={progresso === 0} onClick={() => accoda(libro, progresso - 1)} aria-label="Togli una sessione">−</button>
              <button type="button" className="btn btn-secondary touch" onClick={() => accoda(libro, libro.fatto ? 0 : libro.totaleSessioni)}>{libro.fatto ? 'Azzera' : 'Completa'}</button>
              <button type="button" className="btn btn-ghost touch" disabled={progresso >= libro.totaleSessioni} onClick={() => accoda(libro, progresso + 1)} aria-label="Aggiungi una sessione">+</button>
            ...
            <div className="mt-auto flex flex-wrap gap-2"><button type="button" className="btn btn-ghost btn-sm touch" onClick={() => { setSelezionato(libro.chiave); setFonteSelezionata(0); }}>Mostra posizione</button><a href={libro.fonte} target="_blank" rel="noreferrer" className="credito self-center">fonte</a></div>
          </li>;
        })}
      </ul>
