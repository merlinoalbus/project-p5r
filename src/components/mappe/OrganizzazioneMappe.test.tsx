...
it('il selettore mostra alternative senza default e cambia il contesto esplicitamente',()=>{
  ...
  expect(screen.getAllByRole('option',{name:'Museo, 2P'})).toHaveLength(1);
  fireEvent.change(select,{target:{value:'b|c'}});expect(onCambia).toHaveBeenCalledWith('b|c');
  vista.rerender(<SelettoreContestoMappa mappa={mappa} selezione="a" onCambia={onCambia}/>);
  expect(screen.getByRole('combobox')).toHaveValue('a');expect(screen.queryByRole('list')).not.toBeInTheDocument();
  vista.rerender(<SelettoreContestoMappa mappa={mappa} selezione="errato" onCambia={onCambia}/>);
  expect(screen.getByRole('status')).toHaveTextContent('non appartiene');expect(screen.getByRole('combobox')).toHaveValue('');
});
...
it('il contesto senza titolo rimane selezionato e non è segnalato come estraneo',()=>{
  ...
  expect(screen.getByRole('combobox')).toHaveValue('safe');
  ...
  expect(screen.getByRole('combobox')).toHaveValue('normale');
