...
function spillo(extra: Partial<SpilloDto> & { id: number; nome: string; tipo: SpilloDto['tipo'] }): SpilloDto {
  return { mappaChiave: 'citta-shibuya', tipoNome: extra.tipo, colore: '#abc', descrizione: '', x: 50, y: 50, riferimento: null, collezionabile: false, ordine: 0, origine: 'seed', raccolto: false, dettaglio: null, updatedAt: '2026-09-04T00:00:00.000Z', ...extra };
}
...
const mappa: MappaDto = {
  ...
  spilli: [
    ...
    spillo({ id: 3, nome: 'Verso il centro', tipo: 'passaggio', tipoNome: 'Passaggio', x: 10, y: 90, riferimento: { tipo: 'mappa', chiave: 'shibuya-centro' }, dettaglio: { tipo: 'mappa', mappa: { chiave: 'shibuya-centro', nome: 'Shibuya centro', tipo: 'luogo' } } }),
...
function monta(extra: Partial<Parameters<typeof VisoreMappa>[0]> = {}) {
  ...
  render(<MemoryRouter><VisoreMappa mappa={mappa} partitaId={7} onNaviga={onNaviga} onRaccolto={onRaccolto} {...extra} /></MemoryRouter>);
  return { onNaviga, onRaccolto };
}
...
describe('VisoreMappa', () => {
  it('mostra gli spilli con tipo e nome, nasconde i collezionabili raccolti e li rivela con l’interruttore', () => {
    ...
    // progresso dei collezionabili: 1 su 2
    expect(screen.getByRole('progressbar', { name: 'Collezionabili raccolti' })).toHaveAttribute('aria-valuenow', '50');
