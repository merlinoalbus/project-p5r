...
import type { AzionePercorsoDto, PercorsoGiornoDto, PercorsoIndiceDto } from '../../shared/types.js';
...
export function giornoPercorso(data: string, partitaId?: number): PercorsoGiornoDto {
  ...
  const azioni = (JSON.parse(r.azioni_json) as AzioneSeed[]).map((a, i) => ({ ...a, indice: i, fatta: fatte.has(`${data}/${i}`) }));
...
export function impostaAzione(partitaId: number, data: string, indice: number, fatta: boolean): AzionePercorsoDto {
  partitaEsiste(partitaId);
  ...
  getDb().transaction(() => {
    ...
    if (fatta) prepared('INSERT INTO azione_partita (partita_id, data, indice, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(partita_id, data, indice) DO UPDATE SET updated_at = excluded.updated_at').run(partitaId, data, indice, adesso);
    else prepared('DELETE FROM azione_partita WHERE partita_id = ? AND data = ? AND indice = ?').run(partitaId, data, indice);
    if (fatta && !era) registraEvento(partitaId, 'percorso', `Percorso ${data}: ${a.azione.slice(0, 80)}${a.azione.length > 80 ? '…' : ''}`, `${a.fascia === 'sera' ? 'Sera' : 'Giorno'} · ${a.tipo}${a.riferimentoTesto ? ` · ${a.riferimentoTesto}` : ''}.`, { data, indice });
    prepared('UPDATE partita SET updated_at = ? WHERE id = ?').run(adesso, partitaId);
  })();
  return { ...a, indice, fatta };
}
