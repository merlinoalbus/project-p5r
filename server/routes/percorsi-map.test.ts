...
it('rinomina e sposta un sottoalbero mantenendo pin, progressi e vecchie URL',async()=>{
  ...
  const aggiornato=await request(app).put('/api/mappe/spilli/'+s.id).send({nome:'Tesoro rinominato',riferimento:{tipo:'mappa',chiave:dopo.chiave}});
  expect(aggiornato.status).toBe(200);expect(aggiornato.body.data.riferimento.chiave).toBe(dopo.chiave);
  expect(dopo.chiave).toBe('shinjuku-palazzo-imperiale-piano-0');expect(dopo.spilli[0]).toMatchObject({id:s.id,raccolto:true});
