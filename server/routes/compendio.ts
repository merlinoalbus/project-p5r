...
router.get('/oggetti-guida', (_req, res) => {
  ...
  const ponte = datiGuida<{ abbinamenti: Array<{ nome: string; articolo: string }> }>('oggetti-crosswalk');
  if (ponte) {
    const per = new Map(ponte.abbinamenti.map((a) => [a.nome, a.articolo]));
    for (const v of dati.consumabili ?? []) { const a = per.get(v.nome); if (a) v.articolo = a; }
    for (const v of dati.chiaveEMateriali ?? []) { const a = per.get(v.nome); if (a) v.articolo = a; }
  }
  res.json(dati);
});
