...
it('negozio bloccato si eredita nella scheda, nella ricerca e dopo la spunta',async()=>{
  ...
  const scheda=(await request(app).get(`/api/compendio/negozi/${n.chiave}?partita=${partita}`)).body.data;
  const ricerca=(await request(app).get(`/api/compendio/articoli?q=Prodotto%20test&partita=${partita}`)).body.data;
  expect(scheda.articoliElenco[0].disponibilita.stato).toBe('bloccato');
  expect(ricerca.articoli[0].disponibilita).toEqual(scheda.articoliElenco[0].disponibilita);
  const spuntato=(await request(app).put(`/api/partite/${partita}/acquisti`).send({articolo:a.chiave,fatto:true})).body.data;
  expect(spuntato.disponibilita.stato).toBe('bloccato');
